...
function resetAll(){{ ['q','status','area','country','sort','codeFilter','source'].forEach(id=>$(id).value=''); $('sort').value=DEFAULT_SORT; $('pageSize').value=String(DEFAULT_PAGE_SIZE); page=0; render(); window.scrollTo({{top:0, behavior:'smooth'}}); }}
function chips(){{
...
function restoreStateFromUrl(){{
  ...
  setSelectIfValid('status', params.get('status'));
  setSelectIfValid('area', params.get('area'));
  setSelectIfValid('country', params.get('country'));
  setSelectIfValid('codeFilter', params.get('code'));
  setSelectIfValid('source', params.get('source'));
  setSelectIfValid('sort', params.get('sort') || DEFAULT_SORT);
  setSelectIfValid('pageSize', params.get('pageSize') || String(DEFAULT_PAGE_SIZE));
  pageSize = Number($('pageSize').value || DEFAULT_PAGE_SIZE);
  const urlPage = Number(params.get('page') || 1);
  page = Number.isFinite(urlPage) && urlPage > 0 ? urlPage - 1 : 0;
}}
function syncUrl(){{
  ...
  if ($('sort').value && $('sort').value !== DEFAULT_SORT) params.set('sort', $('sort').value);
  if (Number($('pageSize').value) !== DEFAULT_PAGE_SIZE) params.set('pageSize', $('pageSize').value);
